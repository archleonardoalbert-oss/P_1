...
def Administrar():
    ...
    with col_mid:
        ...
        if st.session_state.Crear:
            ...
            start_date = st.date_input('Ingrese la fecha de inicio del evento', value= date.today(), format= 'DD/MM/YYYY').strftime('%d/%m/%Y')
            end_date = st.date_input('Ingrese la fecha de finalizacion del evento', value= date.today(), format= 'DD/MM/YYYY').strftime('%d/%m/%Y')
            selected_resources = st.multiselect('Que recursos requerira para su evento ?', options= total_resources, key= 'multiselect1')
